...
func spawn_ball_master(p_requester_id, p_entity_callback_id: int) -> void:
	...
	if requester_player_entity:
		var spawned_ball_ref: EntityRef = EntityManager.spawn_entity(\
		interactable_prop_const,\
		{\
		"transform":requester_player_entity.get_last_transform(),
		"model_scene":spawn_model\
		}, "NetEntity", p_requester_id)

func spawn_ball_puppet(p_entity_callback_id: int) -> void:
...
func test_spawning() -> void:
	...
	if spawn_key_pressed_last_frame == false:
		if spawn_key_pressed_this_frame == true:
			spawn_ball()
...
func _entity_ready() -> void:
	get_node(rpc_table).connect("session_master_spawn", self, "spawn_ball_master")
	get_node(rpc_table).connect("session_puppet_spawn", self, "spawn_ball_puppet")
