...
func _on_attempt_grab(p_args: Dictionary) -> void:
	...
	_entity_node.request_reparent_entity(p_args["grabber_entity_ref"], p_args["grabber_attachment_id"])
	_entity_node.send_entity_message(grabber_entity, "pickup_grab_callback", {"entity_ref":_entity_node.get_entity_ref()})

func _on_attempt_drop(p_args: Dictionary) -> void:
	var grabber_entity: EntityRef = p_args["grabber_entity_ref"]
	
	_entity_node.request_reparent_entity(null, 0)
	_entity_node.send_entity_message(grabber_entity, "pickup_drop_callback", {"entity_ref":_entity_node.get_entity_ref()})
